...
//        get(lockAsync(leaseTime, unit));
    ...
    private RFuture<Boolean> tryAcquireOnceAsync(long waitTime, long leaseTime, TimeUnit unit, long threadId) {
        ...
        } else { // 没有设置锁的时间： 使用默认内部看门狗超时时间: 30S
        ...
        CompletionStage<Boolean> f = acquiredFuture.thenApply(acquired -> {
            // lock acquired
            if (acquired) {
                if (leaseTime > 0) { // 当设置了超时时间，则直接使用超时时间
                    internalLockLeaseTime = unit.toMillis(leaseTime);
                } else {
                    // 使用调度器，定时给锁续期： 基于 HashedWheelTimer (可以深入分析)
    ...
    <T> RFuture<T> tryLockInnerAsync(long waitTime, long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
        /**
         * 执行lua脚本来实现锁：
         * 使用了Hash数据结构,存储特点: key field value
         * > key： 就是锁
         * > field: 就说锁的持有者
         * > value: 重入次数
         *
        ...
        return evalWriteSyncedNoRetryAsync(getRawName(), LongCodec.INSTANCE, command,
                "if ((redis.call('exists', KEYS[1]) == 0) " + // 当锁没有被获取
                            "or (redis.call('hexists', KEYS[1], ARGV[2]) == 1)) then " + // 锁被当先线程持有的时候
                        "redis.call('hincrby', KEYS[1], ARGV[2], 1); " + // 加锁成功，设置重入次数
...
//        return get(tryLockAsync(waitTime, leaseTime, unit));
    ...
    protected RFuture<Boolean> unlockInnerAsync(long threadId, String requestId, int timeout) {
        return evalWriteSyncedNoRetryAsync(getRawName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                              "local val = redis.call('get', KEYS[3]); " + // redisson_unlock_latch 是否过期?即表示是否正在解锁
                                    "if val ~= false then " +
                                        "return tonumber(val);" +
                                    "end; " +
                                   ...
                                   // 加锁: 锁被当前线程持有，兼容重入锁逻辑
                                    ...
                                    "if (counter > 0) then " +
                                        ...
                                        "return 0; " +
                                    "else " +
                                        "redis.call('del', KEYS[1]); " + // 删除这个Key
                                        "redis.call(ARGV[4], KEYS[2], ARGV[1]); " + // 发布锁释放消息
                                        "redis.call('set', KEYS[3], 1, 'px', ARGV[5]); " + // 正在解锁标识: lua第一行可知，这个是避免锁释放后其他线程立即获取到锁
                                        "return 1; " +
                                    "end; ",
